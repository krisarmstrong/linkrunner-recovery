...
def parse_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--serial_number", required=True, help="Serial number of the device")
    parser.add_argument(
        "--opt_8021x",
        type=int,
        choices=[0, 1],
    ...
    for drive_letter in string.ascii_uppercase:
        ...
        try:
            if drive_path.is_dir():
                if (
                    drive_path / Config.MARKER_FILE
                ).exists() or Config.VOLUME_LABEL in os.path.basename(str(drive_path)):
...
def main() -> int:
    ...
    try:
        ...
        logging.info(
            "Starting LinkRunner recovery for MAC: %s, Serial: %s",
            args.mac_address,
            args.serial_number,
        )
